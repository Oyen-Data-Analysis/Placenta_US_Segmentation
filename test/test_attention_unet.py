...
def test_unet(model, dataloaders, criterion, model_path):
    ...
    # save epoch losses
    epoch_test_loss = batch_test_loss / len(dataloaders)

    out_tif = torch.stack(outputs_list, dim=0)
    out_tif = torch.flatten(out_tif, start_dim=0, end_dim=1).detach().cpu().numpy()
    to_tiff(x=out_tif, path='/export1/project/h.yuyang/Placenta_OCT_Segmentation/results/06_outputs.tif',
            is_normalized=False)

    label_tif = torch.stack(label_list, dim=0)
    label_tif = torch.flatten(label_tif, start_dim=0, end_dim=1).detach().cpu().numpy()
    to_tiff(x=label_tif, path='/export1/project/h.yuyang/Placenta_OCT_Segmentation/results/06_labels.tif',
            is_normalized=False)


    input_tif = torch.stack(input_list, dim=0)
    input_tif = torch.flatten(input_tif, start_dim=0, end_dim=1).detach().cpu().numpy()
    to_tiff(x=input_tif, path='/export1/project/h.yuyang/Placenta_OCT_Segmentation/results/06_inputs.tif',
            is_normalized=False)
